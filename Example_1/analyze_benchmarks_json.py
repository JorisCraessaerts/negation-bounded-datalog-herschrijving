...
def plot_box_by_rewrite(df_runs: pd.DataFrame, out_dir: Path):
    """Boxplots: total runtime per rewrite, labels als v<nummer>, geen titel/x-label."""
    _maybe_set_style()
    if df_runs.empty:
        return
    ...
    for (dataset, variant), sub in df_runs.groupby(["dataset", "variant"], observed=True):
        ...
        # Map program → v<nummer> op basis van de programnaam zelf (prefix) of rewrite_id
        label_map = {}
        for p in order:
        ...
        if _HAS_SNS:
            sns.boxplot(data=sub, x="prog_short", y="total_runtime_ms")  # geen stripplot
        else:
...
def plot_median_bars(df_runs: pd.DataFrame, out_dir: Path):
    """Balkgrafiek: median runtime per rewrite, labels als v<nummer>, geen titel/x-label."""
    _maybe_set_style()
    if df_runs.empty:
        return
    ...
    for (dataset, variant), sub in med.groupby(["dataset", "variant"], observed=True):
        ...
        # Map program → v<nummer> (uit naam, fallback rewrite_id)
        sub["prog_short"] = [
            _program_to_vlabel(p, rid) for p, rid in zip(sub["program"].astype(str), sub["rewrite_id"].astype(int))
        ]
        # Zorg dat de volgorde op rewrite_id blijft
...
def plot_relation_breakdown(df_rel: pd.DataFrame, df_runs: pd.DataFrame, out_dir: Path, top_n: int = 6):
    """Voor elke (dataset, variant) toon de top-N snelste rewrites (op basis van mediane totale runtime),
       met een stacked bar van mediane relation-runtime per rewrite.
       Labels op de x-as zijn korte versie-etiketten: v<rewrite_id>.
       Verder blijft de logica ongewijzigd.
    """
    _maybe_set_style()
    if df_rel.empty or df_runs.empty:
        return

    # Werk met string key voor programnaam om groupby/categorical hoekjes te vermijden
    runs_tmp = df_runs.copy()
    runs_tmp["program_str"] = runs_tmp["program"].astype(str)

    # Mediane totale runtime per (dataset, variant, program, rewrite_id)
    med = (runs_tmp
    ...
    for (dataset, variant), ss in med.groupby(["dataset", "variant"], observed=True):
        # Neem top-N snelste (laagste median_ms)
        ss = ss.sort_values("median_ms", kind="stable").head(top_n)
        if ss.empty:
            continue

        # Relations-DF voorbereiden (ook program -> str)
        rel_tmp = df_rel.copy()
        ...
        # Mediane relation-runtime per (program_str, relation)
        ...
        # Volgorde van programma's: op rewrite_id (en daarna program_str stabiel)
        ...
        # Gestapelde balken: waarden in dezelfde volgorde als 'progs'
        ...
        # Layout: geen titel, korte labels, legenda buiten de as, geen x-as label
        plt.ylabel("Runtime (ms)")
        plt.xlabel("")  # x-as titel verwijderen
        ...
        # iets ruimere marges onder/ rechts voor labels + legenda
