...
namespace  souffle {
using namespace souffle;
class Sf___laatste_optimalisaties__std: public SouffleProgram {
public:
 Sf___laatste_optimalisaties__std(std::string pf = "profile.log");
 ~Sf___laatste_optimalisaties__std();
void run();
...
namespace  souffle {
using namespace souffle;
 Sf___laatste_optimalisaties__std::Sf___laatste_optimalisaties__std(std::string pf):
...
ProfileEventSingleton::instance().setOutputFile(profiling_fname);
 ...
 Sf___laatste_optimalisaties__std::~Sf___laatste_optimalisaties__std(){
}

void Sf___laatste_optimalisaties__std::runFunction(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
...
void Sf___laatste_optimalisaties__std::run(){
runFunction("", "", false, false);
}

void Sf___laatste_optimalisaties__std::runAll(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
runFunction(inputDirectoryArg, outputDirectoryArg, performIOArg, pruneImdtRelsArg);
}

void Sf___laatste_optimalisaties__std::printAll([[maybe_unused]] std::string outputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"name","result_positive_rewritten"},{"operation","output"},{"output-dir","."},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
void Sf___laatste_optimalisaties__std::loadAll([[maybe_unused]] std::string inputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"fact-dir","."},{"name","edge"},{"operation","input"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
void Sf___laatste_optimalisaties__std::dumpInputs(){
...
void Sf___laatste_optimalisaties__std::dumpOutputs(){
try {std::map<std::string, std::string> rwOperation;
rwOperation["IO"] = "stdout";
...
SymbolTable& Sf___laatste_optimalisaties__std::getSymbolTable(){
return symTable;
}

RecordTable& Sf___laatste_optimalisaties__std::getRecordTable(){
return recordTable;
}

void Sf___laatste_optimalisaties__std::setNumThreads(std::size_t numThreadsValue){
...
void Sf___laatste_optimalisaties__std::executeSubroutine(std::string name,const std::vector<RamDomain>& args,std::vector<RamDomain>& ret){
if (name == "edge") {
stratum_edge_31e40d0d94fe9f7c.run(args, ret);
return;}
...
void Sf___laatste_optimalisaties__std::dumpFreqs(){
}

} // namespace  souffle
namespace souffle {
SouffleProgram *newInstance___laatste_optimalisaties__std(){return new  souffle::Sf___laatste_optimalisaties__std;}
SymbolTable *getST___laatste_optimalisaties__std(SouffleProgram *p){return &reinterpret_cast<souffle::Sf___laatste_optimalisaties__std*>(p)->getSymbolTable();}
} // namespace souffle
...
souffle::Sf___laatste_optimalisaties__std obj(opt.getProfileName());
#if defined(_OPENMP) 
...
namespace  souffle {
using namespace souffle;
class factory_Sf___laatste_optimalisaties__std: souffle::ProgramFactory {
public:
souffle::SouffleProgram* newInstance();
 factory_Sf___laatste_optimalisaties__std();
private:
};
} // namespace  souffle
namespace  souffle {
using namespace souffle;
souffle::SouffleProgram* factory_Sf___laatste_optimalisaties__std::newInstance(){
return new  souffle::Sf___laatste_optimalisaties__std();
}

 factory_Sf___laatste_optimalisaties__std::factory_Sf___laatste_optimalisaties__std():
souffle::ProgramFactory("__laatste_optimalisaties__std"){
}
...
extern "C" {
souffle::factory_Sf___laatste_optimalisaties__std __factory_Sf___laatste_optimalisaties__std_instance;
}
#endif
} // namespace souffle
