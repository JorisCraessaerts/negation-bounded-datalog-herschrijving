...
{
	Logger logger(R"_(@t-relation-loadtime;edge;6_laatste_optimalisaties.dl [1:7-1:11];loadtime;)_",iter, [&](){return rel_edge_04d25f5060e9043b->size();});
if (performIO) {
...
ProfileEventSingleton::instance().makeQuantityEvent( R"(@n-nonrecursive-relation;edge;6_laatste_optimalisaties.dl [1:7-1:11];)",rel_edge_04d25f5060e9043b->size(),iter);}
...
{
	Logger logger(R"_(@t-nonrecursive-relation;result_positive_rewritten;6_laatste_optimalisaties.dl [4:7-4:32];)_",iter, [&](){return rel_result_positive_rewritten_f19b5b85867f266b->size();});
signalHandler->setMsg(R"_(result_positive_rewritten(x,y) :- 
   edge(x,y),
   edge(y,z),
   edge(z,x).
in file 6_laatste_optimalisaties.dl [10:1-13:16])_");
{
	Logger logger(R"_(@t-nonrecursive-rule;result_positive_rewritten;6_laatste_optimalisaties.dl [10:1-13:16];result_positive_rewritten(x,y) :- \n   edge(x,y),\n   edge(y,z),\n   edge(z,x).;)_",iter, [&](){return rel_result_positive_rewritten_f19b5b85867f266b->size();});
...
signalHandler->setMsg(R"_(result_positive_rewritten(x,y) :- 
   ...
   x != y,
   z1 != z2.
in file 6_laatste_optimalisaties.dl [16:1-21:14])_");
{
	Logger logger(R"_(@t-nonrecursive-rule;result_positive_rewritten;6_laatste_optimalisaties.dl [16:1-21:14];result_positive_rewritten(x,y) :- \n   edge(x,y),\n   edge(y,z1),\n   edge(z2,x),\n   x != y,\n   z1 != z2.;)_",iter, [&](){return rel_result_positive_rewritten_f19b5b85867f266b->size();});
if(!(rel_edge_04d25f5060e9043b->empty())) {
[&](){
CREATE_OP_CONTEXT(rel_edge_04d25f5060e9043b_op_ctxt,rel_edge_04d25f5060e9043b->createContext());
CREATE_OP_CONTEXT(rel_result_positive_rewritten_f19b5b85867f266b_op_ctxt,rel_result_positive_rewritten_f19b5b85867f266b->createContext());
...
{
	Logger logger(R"_(@t-relation-savetime;result_positive_rewritten;6_laatste_optimalisaties.dl [4:7-4:32];savetime;)_",iter, [&](){return rel_result_positive_rewritten_f19b5b85867f266b->size();});
if (performIO) {
...
namespace  souffle {
using namespace souffle;
class Sf___laatste_optimalisaties__noopt: public SouffleProgram {
public:
 Sf___laatste_optimalisaties__noopt(std::string pf = "profile.log");
 ~Sf___laatste_optimalisaties__noopt();
void run();
...
namespace  souffle {
using namespace souffle;
 Sf___laatste_optimalisaties__noopt::Sf___laatste_optimalisaties__noopt(std::string pf):
...
ProfileEventSingleton::instance().setOutputFile(profiling_fname);
 ...
 Sf___laatste_optimalisaties__noopt::~Sf___laatste_optimalisaties__noopt(){
}

void Sf___laatste_optimalisaties__noopt::runFunction(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
...
void Sf___laatste_optimalisaties__noopt::run(){
runFunction("", "", false, false);
}

void Sf___laatste_optimalisaties__noopt::runAll(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
runFunction(inputDirectoryArg, outputDirectoryArg, performIOArg, pruneImdtRelsArg);
}

void Sf___laatste_optimalisaties__noopt::printAll([[maybe_unused]] std::string outputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"name","result_positive_rewritten"},{"operation","output"},{"output-dir","."},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
void Sf___laatste_optimalisaties__noopt::loadAll([[maybe_unused]] std::string inputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"fact-dir","."},{"name","edge"},{"operation","input"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
void Sf___laatste_optimalisaties__noopt::dumpInputs(){
...
void Sf___laatste_optimalisaties__noopt::dumpOutputs(){
try {std::map<std::string, std::string> rwOperation;
rwOperation["IO"] = "stdout";
...
SymbolTable& Sf___laatste_optimalisaties__noopt::getSymbolTable(){
return symTable;
}

RecordTable& Sf___laatste_optimalisaties__noopt::getRecordTable(){
return recordTable;
}

void Sf___laatste_optimalisaties__noopt::setNumThreads(std::size_t numThreadsValue){
...
void Sf___laatste_optimalisaties__noopt::executeSubroutine(std::string name,const std::vector<RamDomain>& args,std::vector<RamDomain>& ret){
if (name == "edge") {
stratum_edge_31e40d0d94fe9f7c.run(args, ret);
return;}
...
void Sf___laatste_optimalisaties__noopt::dumpFreqs(){
}

} // namespace  souffle
namespace souffle {
SouffleProgram *newInstance___laatste_optimalisaties__noopt(){return new  souffle::Sf___laatste_optimalisaties__noopt;}
SymbolTable *getST___laatste_optimalisaties__noopt(SouffleProgram *p){return &reinterpret_cast<souffle::Sf___laatste_optimalisaties__noopt*>(p)->getSymbolTable();}
} // namespace souffle
...
{
try{
souffle::CmdOptions opt(R"(6_laatste_optimalisaties.dl)",
R"()",
R"()",
true,
R"(/dev/null)",
...
souffle::Sf___laatste_optimalisaties__noopt obj(opt.getProfileName());
#if defined(_OPENMP) 
...
namespace  souffle {
using namespace souffle;
class factory_Sf___laatste_optimalisaties__noopt: souffle::ProgramFactory {
public:
souffle::SouffleProgram* newInstance();
 factory_Sf___laatste_optimalisaties__noopt();
private:
};
} // namespace  souffle
namespace  souffle {
using namespace souffle;
souffle::SouffleProgram* factory_Sf___laatste_optimalisaties__noopt::newInstance(){
return new  souffle::Sf___laatste_optimalisaties__noopt();
}

 factory_Sf___laatste_optimalisaties__noopt::factory_Sf___laatste_optimalisaties__noopt():
souffle::ProgramFactory("__laatste_optimalisaties__noopt"){
}
...
extern "C" {
souffle::factory_Sf___laatste_optimalisaties__noopt __factory_Sf___laatste_optimalisaties__noopt_instance;
}
#endif
} // namespace souffle
