...
namespace  souffle {
using namespace souffle;
class Sf___unfolding__std: public SouffleProgram {
public:
 Sf___unfolding__std(std::string pf = "profile.log");
 ~Sf___unfolding__std();
void run();
...
namespace  souffle {
using namespace souffle;
 Sf___unfolding__std::Sf___unfolding__std(std::string pf):
...
ProfileEventSingleton::instance().setOutputFile(profiling_fname);
 ...
 Sf___unfolding__std::~Sf___unfolding__std(){
}

void Sf___unfolding__std::runFunction(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
...
void Sf___unfolding__std::run(){
runFunction("", "", false, false);
}

void Sf___unfolding__std::runAll(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
runFunction(inputDirectoryArg, outputDirectoryArg, performIOArg, pruneImdtRelsArg);
}

void Sf___unfolding__std::printAll([[maybe_unused]] std::string outputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"name","result_positive_rewritten"},{"operation","output"},{"output-dir","."},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
void Sf___unfolding__std::loadAll([[maybe_unused]] std::string inputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"fact-dir","."},{"name","edge"},{"operation","input"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
void Sf___unfolding__std::dumpInputs(){
...
void Sf___unfolding__std::dumpOutputs(){
try {std::map<std::string, std::string> rwOperation;
rwOperation["IO"] = "stdout";
...
SymbolTable& Sf___unfolding__std::getSymbolTable(){
return symTable;
}

RecordTable& Sf___unfolding__std::getRecordTable(){
return recordTable;
}

void Sf___unfolding__std::setNumThreads(std::size_t numThreadsValue){
...
void Sf___unfolding__std::executeSubroutine(std::string name,const std::vector<RamDomain>& args,std::vector<RamDomain>& ret){
...
void Sf___unfolding__std::dumpFreqs(){
}

} // namespace  souffle
namespace souffle {
SouffleProgram *newInstance___unfolding__std(){return new  souffle::Sf___unfolding__std;}
SymbolTable *getST___unfolding__std(SouffleProgram *p){return &reinterpret_cast<souffle::Sf___unfolding__std*>(p)->getSymbolTable();}
} // namespace souffle
...
souffle::Sf___unfolding__std obj(opt.getProfileName());
#if defined(_OPENMP) 
...
namespace  souffle {
using namespace souffle;
class factory_Sf___unfolding__std: souffle::ProgramFactory {
public:
souffle::SouffleProgram* newInstance();
 factory_Sf___unfolding__std();
private:
};
} // namespace  souffle
namespace  souffle {
using namespace souffle;
souffle::SouffleProgram* factory_Sf___unfolding__std::newInstance(){
return new  souffle::Sf___unfolding__std();
}

 factory_Sf___unfolding__std::factory_Sf___unfolding__std():
souffle::ProgramFactory("__unfolding__std"){
}
...
extern "C" {
souffle::factory_Sf___unfolding__std __factory_Sf___unfolding__std_instance;
}
#endif
} // namespace souffle
