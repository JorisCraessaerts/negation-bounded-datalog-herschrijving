...
{
	Logger logger(R"_(@t-relation-loadtime;edge;7_original_unfolded.dl [2:7-2:11];loadtime;)_",iter, [&](){return rel_edge_04d25f5060e9043b->size();});
if (performIO) {
...
ProfileEventSingleton::instance().makeQuantityEvent( R"(@n-nonrecursive-relation;edge;7_original_unfolded.dl [2:7-2:11];)",rel_edge_04d25f5060e9043b->size(),iter);}
...
{
	Logger logger(R"_(@t-nonrecursive-relation;result_bounded_negation;7_original_unfolded.dl [6:7-6:30];)_",iter, [&](){return rel_result_bounded_negation_502c0f074595d125->size();});
signalHandler->setMsg(R"_(result_bounded_negation(x,y) :- 
   edge(x,y),
   edge(y,z),
   edge(z,x).
in file 7_original_unfolded.dl [10:1-11:40])_");
{
	Logger logger(R"_(@t-nonrecursive-rule;result_bounded_negation;7_original_unfolded.dl [10:1-11:40];result_bounded_negation(x,y) :- \n   edge(x,y),\n   edge(y,z),\n   edge(z,x).;)_",iter, [&](){return rel_result_bounded_negation_502c0f074595d125->size();});
...
signalHandler->setMsg(R"_(result_bounded_negation(x,y) :- 
   ...
   !edge(y,z2),
   x != y.
in file 7_original_unfolded.dl [14:1-18:12])_");
{
	Logger logger(R"_(@t-nonrecursive-rule;result_bounded_negation;7_original_unfolded.dl [14:1-18:12];result_bounded_negation(x,y) :- \n   edge(x,y),\n   edge(y,z1),\n   !edge(z1,x),\n   edge(z2,x),\n   !edge(y,z2),\n   x != y.;)_",iter, [&](){return rel_result_bounded_negation_502c0f074595d125->size();});
if(!(rel_edge_04d25f5060e9043b->empty())) {
[&](){
CREATE_OP_CONTEXT(rel_edge_04d25f5060e9043b_op_ctxt,rel_edge_04d25f5060e9043b->createContext());
CREATE_OP_CONTEXT(rel_result_bounded_negation_502c0f074595d125_op_ctxt,rel_result_bounded_negation_502c0f074595d125->createContext());
...
if( !(rel_edge_04d25f5060e9043b->contains(Tuple<RamDomain,2>{{ramBitCast(env0[1]),ramBitCast(env2[0])}},READ_OP_CONTEXT(rel_edge_04d25f5060e9043b_op_ctxt)))) {
Tuple<RamDomain,2> tuple{{ramBitCast(env0[0]),ramBitCast(env0[1])}};
rel_result_bounded_negation_502c0f074595d125->insert(tuple,READ_OP_CONTEXT(rel_result_bounded_negation_502c0f074595d125_op_ctxt));
break;
}
...
{
	Logger logger(R"_(@t-relation-savetime;result_bounded_negation;7_original_unfolded.dl [6:7-6:30];savetime;)_",iter, [&](){return rel_result_bounded_negation_502c0f074595d125->size();});
if (performIO) {
...
namespace  souffle {
using namespace souffle;
class Sf___original_unfolded__noopt: public SouffleProgram {
public:
 Sf___original_unfolded__noopt(std::string pf = "profile.log");
 ~Sf___original_unfolded__noopt();
void run();
...
namespace  souffle {
using namespace souffle;
 Sf___original_unfolded__noopt::Sf___original_unfolded__noopt(std::string pf):
...
wrapper_rel_result_bounded_negation_502c0f074595d125(1, *rel_result_bounded_negation_502c0f074595d125, *this, "result_bounded_negation", std::array<const char *,2>{{"i:number","i:number"}}, std::array<const char *,2>{{"x","y"}}, 0),
...
stratum_result_bounded_negation_c45f35cfd3024601(symTable,recordTable,regexCache,pruneImdtRels,performIO,signalHandler,iter,ctr,inputDirectory,outputDirectory,*rel_edge_04d25f5060e9043b,*rel_result_bounded_negation_502c0f074595d125){
...
ProfileEventSingleton::instance().setOutputFile(profiling_fname);
 ...
 Sf___original_unfolded__noopt::~Sf___original_unfolded__noopt(){
}

void Sf___original_unfolded__noopt::runFunction(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
...
void Sf___original_unfolded__noopt::run(){
runFunction("", "", false, false);
}

void Sf___original_unfolded__noopt::runAll(std::string inputDirectoryArg,std::string outputDirectoryArg,bool performIOArg,bool pruneImdtRelsArg){
runFunction(inputDirectoryArg, outputDirectoryArg, performIOArg, pruneImdtRelsArg);
}

void Sf___original_unfolded__noopt::printAll([[maybe_unused]] std::string outputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"name","result_bounded_negation"},{"operation","output"},{"output-dir","."},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
void Sf___original_unfolded__noopt::loadAll([[maybe_unused]] std::string inputDirectoryArg){
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"attributeNames","x\ty"},{"auxArity","0"},{"fact-dir","."},{"name","edge"},{"operation","input"},{"params","{\"records\": {}, \"relation\": {\"arity\": 2, \"params\": [\"x\", \"y\"]}}"},{"types","{\"ADTs\": {}, \"records\": {}, \"relation\": {\"arity\": 2, \"types\": [\"i:number\", \"i:number\"]}}"}});
...
void Sf___original_unfolded__noopt::dumpInputs(){
...
void Sf___original_unfolded__noopt::dumpOutputs(){
try {std::map<std::string, std::string> rwOperation;
rwOperation["IO"] = "stdout";
...
SymbolTable& Sf___original_unfolded__noopt::getSymbolTable(){
return symTable;
}

RecordTable& Sf___original_unfolded__noopt::getRecordTable(){
return recordTable;
}

void Sf___original_unfolded__noopt::setNumThreads(std::size_t numThreadsValue){
...
void Sf___original_unfolded__noopt::executeSubroutine(std::string name,const std::vector<RamDomain>& args,std::vector<RamDomain>& ret){
if (name == "edge") {
stratum_edge_31e40d0d94fe9f7c.run(args, ret);
return;}
...
void Sf___original_unfolded__noopt::dumpFreqs(){
}

} // namespace  souffle
namespace souffle {
SouffleProgram *newInstance___original_unfolded__noopt(){return new  souffle::Sf___original_unfolded__noopt;}
SymbolTable *getST___original_unfolded__noopt(SouffleProgram *p){return &reinterpret_cast<souffle::Sf___original_unfolded__noopt*>(p)->getSymbolTable();}
} // namespace souffle
...
{
try{
souffle::CmdOptions opt(R"(7_original_unfolded.dl)",
R"()",
R"()",
true,
R"(/dev/null)",
...
souffle::Sf___original_unfolded__noopt obj(opt.getProfileName());
#if defined(_OPENMP) 
...
namespace  souffle {
using namespace souffle;
class factory_Sf___original_unfolded__noopt: souffle::ProgramFactory {
public:
souffle::SouffleProgram* newInstance();
 factory_Sf___original_unfolded__noopt();
private:
};
} // namespace  souffle
namespace  souffle {
using namespace souffle;
souffle::SouffleProgram* factory_Sf___original_unfolded__noopt::newInstance(){
return new  souffle::Sf___original_unfolded__noopt();
}

 factory_Sf___original_unfolded__noopt::factory_Sf___original_unfolded__noopt():
souffle::ProgramFactory("__original_unfolded__noopt"){
}
...
extern "C" {
souffle::factory_Sf___original_unfolded__noopt __factory_Sf___original_unfolded__noopt_instance;
}
#endif
} // namespace souffle
